...
public class CardVerificationRepository
{
    ...
    software_code VARCHAR(128) NULL,
    agent_account VARCHAR(128) NULL,
    created_at DATETIME NOT NULL,
    PRIMARY KEY (id),
    KEY idx_card_key_software (software, card_key),
    KEY idx_download_link_id (download_link_id)
            ...
            await EnsureColumnAsync(connection, "software", "ALTER TABLE card_verification_log ADD COLUMN IF NOT EXISTS software VARCHAR(128) NOT NULL DEFAULT '' AFTER id;", cancellationToken).ConfigureAwait(false);
            await EnsureColumnAsync(connection, "software_code", "ALTER TABLE card_verification_log ADD COLUMN IF NOT EXISTS software_code VARCHAR(128) NULL AFTER extraction_code;", cancellationToken).ConfigureAwait(false);
            await EnsureColumnAsync(connection, "agent_account", "ALTER TABLE card_verification_log ADD COLUMN IF NOT EXISTS agent_account VARCHAR(128) NULL AFTER software_code;", cancellationToken).ConfigureAwait(false);
            await EnsureIndexAsync(connection, "card_verification_log", "idx_card_key_software", "CREATE INDEX idx_card_key_software ON card_verification_log (software, card_key);", cancellationToken).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken)
    {
        ...
        await using (var attemptCommand = new MySqlCommand(
                   "SELECT COALESCE(MAX(attempt_number), 0) + 1 FROM card_verification_log WHERE card_key = @CardKey AND software = @Software FOR UPDATE",
                   connection,
                   transaction))
        {
            attemptCommand.Parameters.Add(CreateParameter("@CardKey", MySqlDbType.VarChar, cardKey, 128));
            attemptCommand.Parameters.Add(CreateParameter("@Software", MySqlDbType.VarChar, software, 128));
...
(software, card_key, ip_address, attempt_number, was_successful, download_link_id, download_url, extraction_code, software_code, agent_account, created_at)
VALUES (@Software, @CardKey, @IpAddress, @AttemptNumber, @WasSuccessful, @DownloadLinkId, @DownloadUrl, @ExtractionCode, @SoftwareCode, @AgentAccount, @CreatedAt);",
            connection,
            transaction))
        {
            insertCommand.Parameters.Add(CreateParameter("@Software", MySqlDbType.VarChar, software, 128));
            ...
            insertCommand.Parameters.Add(CreateParameter("@SoftwareCode", MySqlDbType.VarChar, softwareCode, 128));
            insertCommand.Parameters.Add(CreateParameter("@AgentAccount", MySqlDbType.VarChar, agentAccount, 128));
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(query.Software))
        {
            parameters.Add(new ParameterDefinition(MySqlDbType.VarChar, query.Software.Trim(), 128));
            whereBuilder.Append(" AND software = @p").Append(parameters.Count - 1);
        }
        ...
        var dataSql = $"SELECT id, software, card_key, ip_address, attempt_number, was_successful, download_link_id, download_url, extraction_code, created_at FROM card_verification_log {whereBuilder} ORDER BY id DESC LIMIT @p{limitIndex} OFFSET @p{offsetIndex}";
        ...
        await using (var dataCommand = new MySqlCommand(dataSql, connection))
        {
            ...
            while (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
            {
                ...
                var softwareValue = reader.IsDBNull(1) ? string.Empty : reader.GetString(1);
                var cardKeyValue = reader.IsDBNull(2) ? string.Empty : reader.GetString(2);
                var ipValue = reader.IsDBNull(3) ? string.Empty : reader.GetString(3);
                var attemptNumber = reader.IsDBNull(4) ? 0 : reader.GetInt32(4);
                var wasSuccessful = !reader.IsDBNull(5) && reader.GetInt32(5) == 1;
                long? linkId = reader.IsDBNull(6) ? null : reader.GetInt64(6);
                string? url = reader.IsDBNull(7) ? null : reader.GetString(7);
                string? code = reader.IsDBNull(8) ? null : reader.GetString(8);
    ...
    public async Task<IDictionary<long, int>> GetSuccessfulAssignmentsByLinkAsync(string software, CancellationToken cancellationToken)
    {
        await using var connection = await OpenConnectionAsync(cancellationToken).ConfigureAwait(false);
        if (connection is null)
        {
...
WHERE was_successful = 1 AND download_link_id IS NOT NULL AND software = @Software
GROUP BY download_link_id;";

        await using var command = new MySqlCommand(sql, connection);
        command.Parameters.Add(CreateParameter("@Software", MySqlDbType.VarChar, software, 128));
    ...
    public async Task<long?> GetLastSuccessfulLinkForCardAsync(string software, string cardKey, CancellationToken cancellationToken)
    {
        await using var connection = await OpenConnectionAsync(cancellationToken).ConfigureAwait(false);
        if (connection is null)
        {
...
FROM card_verification_log
WHERE card_key = @CardKey AND software = @Software AND was_successful = 1 AND download_link_id IS NOT NULL
ORDER BY id DESC
LIMIT 1;";
    ...
    public async Task<IReadOnlyList<CardDownloadHistoryEntry>> GetDownloadHistoryForCardAsync(string software, string cardKey, CancellationToken cancellationToken)
    {
        await using var connection = await OpenConnectionAsync(cancellationToken).ConfigureAwait(false);
        if (connection is null)
        {
...
ORDER BY id ASC;";
        ...
        CancellationToken cancellationToken)
    {
        ...
        var sqlBuilder = new StringBuilder(@"SELECT card_key,
...
WHERE was_successful = 1");
        ...
        {
            sqlBuilder.Append(" AND software = @Software");
        }
        ...
        if (!string.IsNullOrWhiteSpace(software))
        {
            command.Parameters.Add(CreateParameter("@Software", MySqlDbType.VarChar, software.Trim(), 128));
        }
    ...
    private static async Task EnsureColumnAsync(
        ...
        CancellationToken cancellationToken)
    {
        const string existsSql = @"SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'card_verification_log' AND LOWER(COLUMN_NAME) = @ColumnName";

        await using var command = new MySqlCommand(existsSql, connection);
        command.Parameters.Add(CreateParameter("@ColumnName", MySqlDbType.VarChar, columnName.ToLowerInvariant(), 128));
        var scalar = await command.ExecuteScalarAsync(cancellationToken).ConfigureAwait(false);
        var exists = ConvertToInt64(scalar, 0) > 0;
        if (exists)
        {
            return;
        }
        ...
        catch (MySqlException ex) when (ex.Number == 1060) // ER_DUP_FIELDNAME: duplicate column
        {
            // Another process added the column concurrently.
        }
    }
    ...
    private static async Task EnsureIndexAsync(
        ...
        CancellationToken cancellationToken)
    {
        const string existsSql = @"SELECT COUNT(*) FROM information_schema.STATISTICS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = @Table AND INDEX_NAME = @Index";
        await using var command = new MySqlCommand(existsSql, connection);
        command.Parameters.Add(CreateParameter("@Table", MySqlDbType.VarChar, tableName, 128));
        command.Parameters.Add(CreateParameter("@Index", MySqlDbType.VarChar, indexName, 128));
        var scalar = await command.ExecuteScalarAsync(cancellationToken).ConfigureAwait(false);
        var exists = ConvertToInt64(scalar, 0) > 0;
        if (exists)
        {
        ...
        catch (MySqlException ex) when (ex.Number == (int)MySqlErrorCode.DuplicateKeyName)
        {

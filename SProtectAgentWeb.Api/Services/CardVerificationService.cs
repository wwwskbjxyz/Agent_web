...
public sealed class CardVerificationService
{
    ...
    {
        var normalizedSoftware = software.Trim();
        var normalizedCardKey = cardKey.Trim();
        var normalizedSoftwareCode = string.IsNullOrWhiteSpace(softwareCode) ? null : softwareCode.Trim();
        var normalizedAgentAccount = string.IsNullOrWhiteSpace(agentAccount) ? null : agentAccount.Trim();
        CardInfo? card;
        try
        {
            card = await _cardService.GetCardByKeyAsync(normalizedSoftware, normalizedCardKey, cancellationToken);
        }
        catch (Exception ex)
        {
            ...
            var attemptNumber = await _repository.RecordAttemptAsync(
                ...
                downloadUrl: null,
                extractionCode: null,
                normalizedSoftwareCode,
                normalizedAgentAccount,
                cancellationToken: cancellationToken);
        ...
        {
            var existingHistory = await LoadDownloadHistorySafeAsync(normalizedSoftware, normalizedCardKey, cancellationToken);
            ...
            {
                assignedLink = await TryAssignDownloadLinkAsync(normalizedSoftware, normalizedCardKey, usedLinkIds, cancellationToken);
    ...
    private async Task<LanzouLinkInfo?> TryAssignDownloadLinkAsync(
        string software,
        string cardKey,
        ISet<long> usedLinkIds,
        CancellationToken cancellationToken)
    {
        var links = await _lanzouLinkService.GetAvailableLinksAsync(software, cancellationToken);
        if (links.Count == 0)
        {
        ...
        try
        {
            usageCounts = await _repository.GetSuccessfulAssignmentsByLinkAsync(software, cancellationToken);
            lastLinkId = await _repository.GetLastSuccessfulLinkForCardAsync(software, cardKey, cancellationToken);
        }
        catch (Exception ex)
        {
    ...
    private async Task<IReadOnlyList<CardDownloadHistoryEntry>> LoadDownloadHistorySafeAsync(
        string software,
        string cardKey,
        CancellationToken cancellationToken)
    {
        try
        {
            return await _repository.GetDownloadHistoryForCardAsync(software, cardKey, cancellationToken);
        }
        catch (Exception ex)
        {

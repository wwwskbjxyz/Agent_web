...
public class LinkAuditService
{
    ...
    {
        var software = ResolveSoftware(request.Software);
        var (items, total) = await _lanzouLinkService.QueryLinksAsync(software, request.Keyword, request.Page, request.Limit, cancellationToken);
    ...
    {
        var software = ResolveSoftware(request.Software);
        return await _lanzouLinkService.DeleteLinksAsync(software, request.Ids, cancellationToken);
    }

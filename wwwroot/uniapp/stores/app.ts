...
function transformSalesResult(data?: any): SalesQueryResultPayload {
  ...
  return { count: Number.isFinite(count) ? count : cards.length, cards };
}
...
export const useAppStore = defineStore('app', () => {
  ...
  async function loadSoftwareList() {
    ...
    try {
      ...
      const stored = getSelectedSoftwareName();
      ...
      if (matched) {
        ...
        setSelectedSoftwareName(matched.softwareName);
      } else if (items.length) {
        selectedSoftware.value = items[0].softwareName;
        setSelectedSoftwareName(items[0].softwareName);
      } else {
        selectedSoftware.value = '';
        setSelectedSoftwareName();
      }
  ...
  function setSelectedSoftware(name: string) {
    ...
    setSelectedSoftwareName(match.softwareName);
  }
  ...
  function clearSalesResult(nextFilters?: Partial<SalesQueryFilters>) {
    dashboard.salesResult = { count: 0, cards: [] };
  ...
  async function loadVerification(cardKey?: string) {
    ...
    try {
      ...
      const response = await apiRequest<CardVerificationResult>({
        url: '/api/CardVerification/verify',
        method: 'POST',
        data: { cardKey },
        disableMock: true
      });
